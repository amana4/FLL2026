# Robot Design Judging Prep

Rubric criteria below are the **official BIOGLOW wording** from
`rubrics-color.pdf` (page 3). Judges tick one box per row:
**Beginning (1) · Developing (2) · Accomplished (3) · Exceeds (4)**.

> Some Robot Design rows **count dually toward Core Values**. Core Values are
> 25% of the Champion's score, so how you work is being scored here too.

## The five criteria, and what "Accomplished" requires

| Criterion | To score Accomplished you need… | Our evidence |
| --- | --- | --- |
| **IDENTIFY** — decided which missions to attempt, explored resources, sought guidance | Clear evidence of mission strategy **+** clear use of building/coding resources supporting it | _TODO_ |
| **DESIGN** — worked collaboratively, built the needed skills | Clear evidence **all** members contributed ideas **+** all have building and coding skills | _TODO_ |
| **CREATE** — original designs or improvements matching the strategy | Clear explanation of **innovative** attachments **+** innovative code/sensor use | _TODO_ |
| **ITERATE** — repeatedly tested, improved from findings | Clear evidence of **repeated** testing **+** improvements based on it | _TODO_ |
| **COMMUNICATE** — explained learning, celebrated progress | Detailed explanation of process and lessons **+** visible pride/enthusiasm | _TODO_ |

Note what recurs: **every criterion has an "all team members" or "clear
evidence" bar.** One student who knows everything scores lower than five
students who each know their part.

## What to bring

- [ ] The robot
- [ ] Attachments, and a live demo of a swap
- [ ] [`design-log.md`](../../robot-design/design-log.md) — printed. This *is*
      the ITERATE evidence.
- [ ] Block screenshots from [`code/screenshots/`](../../code/screenshots/README.md)
- [ ] Photos of earlier versions that didn't work
- [ ] Reliability numbers from the run files
- [ ] Mission strategy: which of the 15 you chose, and why

## Who explains what

Rotate during practice — the rubric explicitly rewards *all* members having
building and coding skills. The running record of who has actually done what is
in [`who-did-what.md`](../who-did-what.md); an empty row there is a question
somebody cannot answer in the room.

| Topic | Lead | Backup |
| --- | --- | --- |
| Mission strategy (which of the 15, and why) | _TODO_ | _TODO_ |
| Chassis and drivetrain | _TODO_ | _TODO_ |
| Sensors and navigation | _TODO_ | _TODO_ |
| Attachments | _TODO_ | _TODO_ |
| Programming and My Blocks | _TODO_ | _TODO_ |
| Testing process | _TODO_ | _TODO_ |

## Questions to be ready for

| Question | Our answer |
| --- | --- |
| Walk us through your robot. | _TODO_ |
| Which missions did you choose, and why those? | _TODO_ |
| Why did you skip the others? | _TODO_ — cite points-per-second, not "too hard" |
| Which dock did you put each model on, and why? | _TODO_ — M13–M15 strategy |
| How do you make it drive straight? | _TODO_ |
| How do you get consistent turns? | _TODO_ |
| How do you know it's reliable? | _TODO_ — quote the actual percentages |
| What broke, and how did you fix it? | _TODO_ |
| Show us something you redesigned. | _TODO_ |
| How is your code organised? | _TODO_ — My Blocks are the answer |
| What resources did you use? | _TODO_ — SPIKE app guided mission, videos, other teams |
| Who wrote which part? | _TODO_ |
| What would you improve with more time? | _TODO_ |

## Numbers to have memorised

Vague answers read as "we didn't measure."

| Thing | Value |
| --- | --- |
| Wheel circumference | _TODO_ |
| Typical run success rate | _TODO_ |
| Best practice score | _TODO_ |
| Attachment swap time | _TODO_ |
| Missions attempted (of 15) | _TODO_ |
| Motors used (max 4) | _TODO_ |

## Demo checklist

- [ ] Robot charged
- [ ] Programs loaded and tested
- [ ] Attachments in a tidy box
- [ ] Practised explaining the robot **without a field present** — there is no
      robot game field in the judging room, so you cannot demonstrate a mission.
      Bring photos and the design log instead.

--8<-- "includes/abbreviations.md"
