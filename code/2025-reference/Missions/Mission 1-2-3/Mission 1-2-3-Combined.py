...
#start from box 10, the claw aligned with mission 1 key

async def run_mission1():
    #await motor.run_for_degrees(port.C, 49, 200)
    # Move forward
    await drive_cm(-58, velocity=1000)
    # Turn
    await turn_deg(-70, velocity=1000)
    #Move forward
    await drive_cm(-5, velocity=1000)
    # hit
    await turn_deg(-70, velocity=1000)
    await turn_deg(70, velocity=1000)
    #Drive back
    await drive_cm(5, velocity=200)
    await turn_deg(70, velocity=1000)
    await drive_cm(-7,velocity=1000)
    # start mission 2 (the first part)
    await turn_deg(-60)
    await motor.run_for_degrees(port.C, -49, 200)
    await drive_cm(-13)
    await turn_deg(20)
    await turn_deg(-20)
    #start the 2nd part of mission 2
    await motor.run_for_degrees(port.C, 49, 200)
    await turn_deg(34)
    await motor.run_for_degrees(port.C, -40, 200)
    await drive_cm(-4)
    #start 3rd part of mission 2
    await drive_cm(5)
    await motor.run_for_degrees(port.C, 40, 200)
    await drive_cm(5)
    await turn_deg(10)
    ...
    #transition to pick up mission 1
    await drive_cm (20)
    await motor.run_for_degrees(port.C, -45, 200)
    await turn_deg (-53)
    #pick up mission 1
    await drive_cm (-8, velocity= 200)
    ...
    #Bring down front attachement
    await motor.run_for_degrees(port.D, -300, 700)

    #Drive toward mission3
    await drive_cm (26,400)
    # # # lift up the tire claw to make the person on the cart go down
    await motor.run_for_degrees(port.D, 200, 80)
    await runloop.sleep_ms(800)
    await motor.run_for_degrees(port.D, 100, 80)
    ...
    await drive_cm(-21,800)
    await turn_deg (-47)
    await drive_cm(-50,1000)
