# Mission 8_6; this code will first attempt to complete Mission 8 then immediate move on to Misstion 6 - optimization trial
...
def _robot_deg_to_wheel_deg(robot_deg: float) -> int:
    ...
    # wheel_rot = abs(robot_deg) / 360.0) * TRACK_W_MM / (WHEEL_D_MM)
    return int(round(abs(robot_deg) * TRACK_W_MM / WHEEL_D_MM))
...
async def run_mission8_6():
    ...
    await drive_cm(26,velocity=1100)

    # Slap silo 4 times
    for i in range(4):
        await run_attachment_deg(attachment2, -attachment_distance, velocity=1110)
        await run_attachment_deg(attachment2, attachment_distance, velocity=1110)
    ...
    await turn_deg(80)
    await drive_cm(7)
    await run_attachment_deg(attachment2, -180, velocity=600) # attachment going down to hit the red thing to make the boulders down.
    ...
    await turn_deg(20)
    await drive_cm(-65)
    await run_attachment_deg(attachment2, 250, velocity=600)
    await drive_cm(30)
    await drive_cm(-40)
...
async def mission_10():
    # await run_attachment_deg(attachment2, 150, velocity=100)
    await turn_deg(-63)
    await drive_cm(41)
    await run_attachment_deg(attachment2, -250, velocity=1100)
...
async def run_mission8_6_gemini_trick():
    ...
    # # attachment should start at the highest point
    await drive_cm(26,velocity=1100)
    await run_attachment_deg(attachment2, 80, velocity=600) # attachment going down to hit the red thing to make the boulders down.
    ...
    await turn_deg(85)
    await drive_cm(14)
    # await run_attachment_deg(attachment2, -170, velocity=600)
    await turn_deg(-40)
    await drive_cm(5)
    await turn_deg(-35)
...
async def main():
    ...
    # await run_mission8_6_gemini_trick()
    # await clean_robot()
