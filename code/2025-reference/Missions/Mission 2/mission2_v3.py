...
#robot start at block 10
async def run_mission2():
    # first push
    # aim the left claw at the red handle(stick)
    # the claw has to be loose and touching the ground when start
    # from the base to the very back of the robot is 37 cm(14.5 in.)
    # start at the tenth block
    await drive_cm(-63,velocity=400)
    await turn_deg(-49)
    await drive_cm(-16, velocity=200)
    await turn_deg(10)# Make sure the gap is closed
    await turn_deg(-10)

    await drive_cm(17, velocity= 200)
    await motor.run_for_degrees(port.C, 40, 200)    # Lift the attachment
    await turn_deg(90)
    await drive_cm(-8)
    await turn_deg(-90) # Get ready for the next push

    # second push
    await drive_cm(-13, velocity= 100)
    await motor.run_for_degrees(port.C, -37, 200)# Lower the attachment
    await drive_cm(-6, velocity=100)
    await motor.run_for_degrees(port.C, 37, 200)    # Lift the attachment
    await drive_cm(20)# Back up after the attachment is lifted
    await turn_deg(7)# Get ready for the third step

    # lifting the topsoil
    await drive_cm(-15)
    await motor.run_for_degrees(port.C, 73, 150)
    await drive_cm(10)
    #Come back
    await turn_deg(49, velocity=200)
    await drive_cm(65)

# -----------------------------
# Main program, start from block 10
# -----------------------------
async def main():
