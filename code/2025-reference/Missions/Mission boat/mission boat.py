...
async def run_mission_boat():
    #bumper aligned on the left outer line of the 6th box, use right rear yellow hole to align with corner, right
    #black bumper align with black line

    # Drive to red handle and pull
    await drive_cm(-32)
    await motor.run_for_degrees(port.C, -20, 600)
    await drive_cm(7, velocity=1000)

    # Release handle, turn to push boat
    await motor.run_for_degrees(port.C, 30, 600)
    await turn_deg(-15)
    await drive_cm(-10)

    # lower attachment to get into position to push boat
    await motor.run_for_degrees(port.C,-6, 600)
    await drive_cm(-10)
    await drive_cm(40)
