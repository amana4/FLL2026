...
# with:
#
#     python3 tools/build-mission.py code/missions/M02-exploding-seeds/mission-advanced.py
#
...
#
# M02 — Exploding Seeds, on the advanced library.
#
...
#
# Scoring, from robot-game/missions/M02-exploding-seeds.md:
#   10 points per seed knocked off the stalk, 3 seeds, 30 maximum.
#
# The attachment is on the BACK of the robot, which is why the approach to the
# model is a reverse. That is not a mistake.
#
# Turns use face(), not turn_deg_gyro(). face() takes a bearing measured from
# base, so a turn that lands half a degree short does not hand that half degree
# to the next one. See code/learn/gyro-correction.html.


# -----------------------------
# Numbers to tune
# -----------------------------
# Every number the run depends on, in one place. Change them here, not inline.
MAT_CALIBRATION = 179 / 180     # 179 cm measured for 180 cm commanded
APPROACH_BEARING = 52.5         # degrees from base, clockwise, towards the stalk
BASE_BEARING = 0.0              # the way the robot was placed in base
REVERSE_CM = 45.0               # back up to the stalk
RETURN_CM = 44.9                # and forward again, 1 mm shy on purpose
LIFT_DEG = -150                 # attachment C, raise the key
LOWER_DEG = 150                 # attachment C, put it back
ATTACH_SPEED = 1000

# How far off a bearing we are willing to be before the log shouts about it.
BEARING_WARN = 2.0
...
# Logging
# -----------------------------
# Set False for a competition run. The prints cost a few milliseconds each and
# the hub console is not there on the table anyway.
LOG = True

_step = [0]

# The hub console is short and it scrolls. A full run pushes its own first half
# off the top, which is how the reverse leg's numbers were lost on the
# 20 September run. So every measurement is kept here and printed again as a
# compact summary at the end. The last thing printed is the thing you can read.
_turns = []      # (label, wanted bearing, actual bearing)
_drives = []     # (label, asked cm, target cm, wheels cm, allowance cm)


def say(what):
    """Print BEFORE doing something, so a step that fails still names itself.

    Everything else here prints after the action. That is fine when the action
    works and useless when it does not: the log simply stops and you are left
    guessing which line died. This one goes first.
    """
    if not LOG:
        return
    _step[0] += 1
    print("%2d  start   %s" % (_step[0], what))


def log_step(what, want=None):
    """Print where the robot thinks it is pointing, after a move.

    `want` is the bearing the step was aiming for. Leave it out for a step that
    is not supposed to change the heading, like a drive, and the log shows the
    heading it held instead.
    """
    if not LOG:
        return
    _step[0] += 1
    here = bearing()
    if want is None:
        print("%2d  %-22s bearing %+7.1f" % (_step[0], what, here))
    else:
        _turns.append((what, want, here))
        off = here - want
        flag = "   <-- CHECK THIS" if abs(off) > BEARING_WARN else ""
        print("%2d  %-22s bearing %+7.1f   want %+6.1f   off %+5.1f%s"
              % (_step[0], what, here, want, off, flag))


def log_travel(what, commanded_cm):
    """Print what the wheels actually turned, against what the move aimed for.

    Three numbers, and the middle one matters.

      asked   what this file passed to drive_cm
      target  what drive_cm aimed for after taking off the coast allowance
      wheels  what the encoders actually turned

    A gap between asked and target is NOT a fault. It is the distance the robot
    is expected to coast after the brakes go on. A gap between target and wheels
    IS worth looking at: that one means the wheel diameter or the calibration
    scale is wrong, and the gyro has nothing to do with it.

    _drive_degrees() is zeroed at the start of every move, so call this straight
    after one.
    """
    if not LOG:
        return
    got_cm = _deg_to_cm(_drive_degrees())
    target_cm = _LAST_DRIVE_TARGET_CM
    _drives.append((what, commanded_cm, target_cm, got_cm, _LAST_DRIVE_COMP_CM))
    _step[0] += 1
    print("%2d  %-22s asked %5.1f  target %5.1f  wheels %5.1f  coast %+5.2f cm"
          % (_step[0], what, commanded_cm, target_cm, got_cm, got_cm - target_cm))


def log_summary():
    """Reprint every measurement at the end, compactly.

    The hub console is short and it scrolls, so a full run pushes its own first
    half off the top. On 20 September that lost the reverse leg's numbers. This
    runs last, which makes it the part you can actually read.

    It also works out what STOP_COMP_FWD_CM wants to be, from the coast the
    robot actually did rather than from a guess.
    """
    if not LOG:
        return
    ...
    if _turns:
        print("  bearings                  want     got     off")
        for label, want, got in _turns:
            flag = "  <-- CHECK" if abs(got - want) > BEARING_WARN else ""
            print("    %-22s %+6.1f  %+6.1f  %+6.1f%s"
                  % (label, want, got, got - want, flag))

    if _drives:
        print("  drives                   asked  target  wheels   coast")
        total_coast = 0.0
        for label, asked, target, wheels, _allowance in _drives:
            coast = wheels - target
            total_coast += coast
            print("    %-22s %5.1f   %5.1f   %5.1f   %+5.2f"
                  % (label, asked, target, wheels, coast))

        coast = total_coast / len(_drives)
        # The library scales the constant by speed before using it, so work back
        # through the allowance it actually applied rather than re-deriving it.
        allowance = _drives[-1][4]
        print("")
        print("  average coast %+.2f cm against an allowance of %.2f cm" % (coast, allowance))
        if allowance > 0:
            wanted = STOP_COMP_FWD_CM * coast / allowance
            print("  so STOP_COMP_FWD_CM wants to be about %.1f, not %.1f"
                  % (wanted, STOP_COMP_FWD_CM))
            print("  only believe that from a run on the real robot. The pretend")
            print("  hub brakes dead, so it always reports almost no coast.")
            print("  and do it twice. One run is not a measurement.")
...
async def run_mission():
    """Reverse into the stalk, knock the seeds off, come home.

    Bearings, not relative turns. Both turns name an absolute direction, so the
    second one cleans up whatever the first one left behind.

    Every step says what it is about to do before it does it. If the log stops,
    the last "start" line is the step that failed, and the except block below
    prints why.
    """
    log_step("in base", BASE_BEARING)

    say("face the stalk")
    await face(APPROACH_BEARING)
    log_step("faced the stalk", APPROACH_BEARING)

    # Raise the key before backing in, so it clears the model on the way.
    #
    # move_attachment_deg, not motor.run_for_degrees. LIFT_DEG is negative, and
    # the raw SPIKE call hangs forever on a negative degree count. The library
    # translates the sign into the velocity instead. That hang is what stopped
    # the 20 September run dead after this line.
    say("raise the key on %s" % port_label(attachment1))
    await move_attachment_deg(attachment1, LIFT_DEG, ATTACH_SPEED)
    log_step("key up")

    # Back into the stalk. The attachment is on the rear.
    say("reverse %.1f cm into the stalk" % REVERSE_CM)
    entry = bearing()
    await drive_cm(-REVERSE_CM)
    log_travel("reversed to stalk", REVERSE_CM)
    # Judged against `entry`, not APPROACH_BEARING. A drive holds the heading it
    # starts on. Comparing it to the bearing face() aimed for mixes the turn's
    # error into the drive's, and hides how much the drive actually leaked.
    log_step("after reversing", entry)

    # Drop the key. This is the action that knocks the seeds off.
    say("drop the key, knock the seeds off")
    await move_attachment_deg(attachment1, LOWER_DEG, ATTACH_SPEED)
    log_step("key down, seeds away")

    # Come forward again.
    say("come forward %.1f cm" % RETURN_CM)
    entry = bearing()
    await drive_cm(RETURN_CM)
    log_travel("returned from stalk", RETURN_CM)
    log_step("after returning", entry)

    # Square back up to the way we started. With face() this also absorbs any
    # heading the two drives lost along the way.
    say("square up to base")
    await face(BASE_BEARING)
    log_step("squared up to base", BASE_BEARING)
...
# -----------------------------
async def main():
    # default_speed=1000 is about 95 percent. Fast, and the least accurate
    # setting there is. Leave it out entirely and drive_cm picks 60 percent,
    # or 30 percent for a short move. Worth trying if the run is inconsistent.
    await init_robot(default_speed=1000)

    if LOG:
        print("M02 Exploding Seeds")
        print("calibration scale:", set_calibration_scale(MAT_CALIBRATION))
        print("wheel:", WHEEL_D_MM, "mm    YAW_SIGN:", YAW_SIGN)
        print("drive motors on", port_label(LEFT_DRIVE), "and", port_label(RIGHT_DRIVE))
        print("attachment on", port_label(attachment1),
              "- motor present:", _has_motor(attachment1))
        print("step  what")
    else:
        set_calibration_scale(MAT_CALIBRATION)

    # A failing step used to leave the log simply stopping, with no reason given.
    # This names the step and prints what went wrong, then still reports the
    # heading so the run is not a total loss.
    failed = None
    try:
        await run_mission()
        ...
        failed = exc
        print("")
        print("STOPPED at step %d." % _step[0])
        print("reason: %s: %s" % (type(exc).__name__, exc))
        print("scroll up to the last 'start' line - that is the step that died.")

    if LOG:
        drift = bearing() - BASE_BEARING
        print("")
        print("finished %+.1f degrees off the bearing it started on." % drift)
        if failed is None and abs(drift) > BEARING_WARN:
            print("that is more than %.1f. Check the last face() and the mat grip."
                  % BEARING_WARN)
        elif failed is not None:
            print("the run did not complete, so that number means nothing yet.")
        log_summary()
